...
class User(Interface):
    ...
    def to_python(cls, data):
        ...
        if 'data' not in data:
            body_data = {k: v for k, v in six.iteritems(orig) if k not in data}
            if body_data:
                data['data'] = body_data

        trim_keys = {'id', 'email', 'username', 'ip_address', 'name', 'data'}
        data = {k: trim(data.get(k, None), 128) for k in trim_keys}
        return cls(**data)
